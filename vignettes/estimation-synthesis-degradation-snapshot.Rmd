...
# Use PlotScatter
# Fig4A
PlotScatter(df.old, remove.outlier = 999) + xlab("LFC(old RNA): perturbed delta vs control") + ylab("LFC(old RNA): unperturbed vs control")
```
...
PlotScatter(df.new, highlight = short.lived.genes, remove.outlier = 999) + xlab("LFC(new RNA): perturbed delta vs control") + ylab("LFC(new RNA): unperturbed vs control")
```
...
```{r}
# Fig4C
PlotScatter(df.hl, remove.outlier = 999) + geom_abline() + xlab("True LFC(half-life)") + ylab("MAP estimate of LFC(half-life)")
```

```{r}
# Fig4D
PlotScatter(df.s, remove.outlier = 999) + geom_abline() + xlab("True LFC(synthesis rate)") + ylab("MAP estimate of LFC(synthesis rate)")
```

(This part until approx.l 261 can probably be deleted.)
...
ggplot(box.hl.molten, aes(x = variable, y = value)) + xlab("simulation") + ylab("delta") + geom_boxplot() + scale_x_discrete(labels = c("unperturbed vs control", "perturbed delta vs control", "perturbed sigma vs control"))
```
Sigma: 
...
# Fig4E (sigma)
ggplot(box.s.molten, aes(x = variable, y = value)) + xlab("simulation") + ylab("sigma") + geom_boxplot() + scale_x_discrete(labels = c("unperturbed vs control", "perturbed delta vs control", "perturbed sigma vs control"))
```
Once again, we can explore how these plots are affected by gene filtering.
(Probably can also be deleted: up to approx l 330)
...
ggplot(box.hl.molten.filtered, aes(x = variable, y = value)) + geom_boxplot() 
```
...
sum(values.s < max(upper.s, lower.s) & values.s > min(lower.s, upper.s)) # 10835, why?

count.s <- 0
for (i in 1:length(values.s)) {
  value.s.i <- values.s[i]
  # this because upper.s has at least some negative values where lower.s has positive ones
  max.s.i <- max(upper.s[i], lower.s[i])
  min.s.i <- min(upper.s[i], lower.s[i])
  if (value.s.i < max(max.s.i, min.s.i) & value.s.i > min(max.s.i, min.s.i)) {
    count.s <- count.s + 1
  }
}

# Instead of for loop
# What's going on with the signs?
sum(values.d > upper.d & values.d < lower.d) # 10815

count.d <- 0
for (i in 1:length(values.d)) {
  value.d.i <- values.d[i]
  # this because upper.s has at least some negative values where lower.s has positive ones
  max.d.i <- max(upper.d[i], lower.d[i])
  min.d.i <- min(upper.d[i], lower.d[i])
  if (value.d.i < max(max.d.i, min.d.i) & value.d.i > min(max.d.i, min.d.i)) {
    count.d <- count.d + 1
  }
}
count.s # 10827 
...
# As expected, d is harder to estimate: around 8000 out of 11000 genes
# were estimated correctly, as opposed to around 10000 for synthesis rate
# Credible intervals are too big, more than 95% of genes fulfil the condition
# upper and lower are switched
# omit fig4f, write results as percentage values
```
...
ggroc(list("ROPE" = roc.rope.old.d, "DESeq2" = roc.deseq.old.d), legacy.axes = TRUE) + ggtitle("Estimate of delta - old RNA")
```
...
ggroc(list("ROPE" = roc.rope.new.d, "DESEq2" = roc.deseq.new.d), legacy.axes = TRUE) + ggtitle("New RNA - estimate of delta")
```
...
ggroc(list("ROPE" = roc.rope.old.s, "DESeq2" = roc.deseq.old.s), legacy.axes = TRUE) + ggtitle("Old RNA - estimate of sigma")
```
...
ggroc(list("ROPE" = roc.rope.new.s, "DESeq2" = roc.deseq.new.s), legacy.axes = TRUE) + ggtitle("New RNA - estimate of sigma")
...
```{r}
# Code. Same as before, but for a full time course.
# Using many time points proves the point more strongly.
seed = 1337
f0 <- GetAnalysisTable(sars)$kinetics.Mock.Synthesis/log(2)*GetAnalysisTable(sars)$`kinetics.Mock.Half-life`
control.full <- SimulateTimeCourse("control", GeneInfo(sars),
                           s = GetAnalysisTable(sars)$kinetics.Mock.Synthesis,
                           d = log(2)/GetAnalysisTable(sars)$`kinetics.Mock.Half-life`,
                           dispersion = estimate.dispersion(GetTable(sars,type="count")),
                           f0 = f0,
                           s.variation = 1.05,
                           d.variation = 1.05,
                           timepoints = c(0, 0, 0, 1, 1, 1, 2, 2, 2, 4, 4, 4, 6, 
                                          6, 6, 8, 8, 8),
                           beta.approx = TRUE, num.reads = 5E7, seed = seed)
unperturbed.full <- SimulateTimeCourse("unperturbed", GeneInfo(sars),
                                 s = GetAnalysisTable(sars)$kinetics.Mock.Synthesis,
                                 d = log(2)/GetAnalysisTable(sars)$`kinetics.Mock.Half-life`,
                                 dispersion=estimate.dispersion(GetTable(sars,type="count")),
                                 f0 = f0,s.variation = 1.05,d.variation = 1.05,timepoints = c(0, 0, 0, 1, 1, 1, 2, 2, 2, 4, 4, 4, 6, 6, 6, 8, 8, 8),
                                 beta.approx = TRUE,
                                 num.reads=5E7,
                                 seed=seed*13)
perturbed.d.full <- SimulateTimeCourse("HL", GeneInfo(sars),
                                 s = GetAnalysisTable(sars)$kinetics.Mock.Synthesis,
                                 d = log(2)/GetAnalysisTable(sars)$`kinetics.Mock.Half-life` *2^rnorm(nrow(sars),0,1) ,
                                 dispersion = estimate.dispersion(GetTable(sars,type="count")),
                                 f0 = f0, s.variation = 1.05, d.variation = 1.05,
                                 timepoints = c(0, 0, 0, 1, 1, 1, 2, 2, 2, 4, 4, 4, 6, 
                                          6, 6, 8, 8, 8), beta.approx = TRUE,
                                 num.reads=5E7, seed = seed*13*13)
perturbed.s.full <- SimulateTimeCourse("s", GeneInfo(sars),
                                 s = GetAnalysisTable(sars)$kinetics.Mock.Synthesis *2^rnorm(nrow(sars),0,1) ,
                                 d = log(2)/GetAnalysisTable(sars)$`kinetics.Mock.Half-life`,
                                 dispersion = estimate.dispersion(GetTable(sars,type="count")),
                                 f0 = f0, s.variation = 1.05, d.variation = 1.05,
...
saveRDS(control.full, "reg.control.full.rds")
saveRDS(unperturbed.full, "reg.unperturbed.full.rds")
saveRDS(perturbed.s.full, "reg.perturbed.s.full.rds")
saveRDS(perturbed.d.full, "reg.perturbed.d.full.rds")

m.full <- merge(control = control.full, unperturbed = unperturbed.full, perturbed.d = perturbed.d.full, perturbed.s = perturbed.s.full)
# In GeneInfo, they will be called true_d.name, true_s.name
saveRDS(m.full, "regulation.full.timecourse.rds")
```

They're basically the same (fig5).

I think the point is to focus on the d and s values of the d and s pertubations and compare those to the d and s values to those of the
corresponding simulations from the snapshot.
