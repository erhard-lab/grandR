...
# 9162 genes remain, rather than 10835 as before.
```

Subset the 2h time point:
```{r}
sars.2h <- subset(sars, columns = c("Mock.2h.A", "Mock.2h.B", "SARS.2h.A", "SARS.2h.B"))
```

Normalize both:
```{r}
sars <- Normalize(sars)
sars.2h <- Normalize(sars.2h)
```

Estimate regulation in the 2h snapshot:
```{r}
SetParallel()
sars.2h <- EstimateRegulation(data = sars, name = "Regulation", contrasts = GetContrasts(sars, contrast = c("Condition","Mock")), reference.columns = FindReferences(sars, Condition == "Mock"), verbose = TRUE,
                              sample.f0.in.ss = TRUE)
```

But the counts are not zero:

```{r}
View(head(GetData(sars.2h, mode.slot = "count")))
```

Estimate regulation in the full time course:

```{r}
SetParallel()
sars <- EstimateRegulation(data = sars, name = "Regulation", contrasts = GetContrasts(sars, contrast = c("Condition","Mock"), group = "duration.4sU"), reference.columns = FindReferences(sars, Condition == "Mock", group = "duration.4sU"), verbose = TRUE, sample.f0.in.ss = TRUE)
saveRDS(sars, "regulation.full.timecourse.rds")
```
Make the x and y axis for synthesis and degradation rate:
(Do we need synthesis rates? The manuscript only mentions degradation
for plotting, but maybe we need synthesis for commentary.)
...
# Full time course
sigma.x <- anal.table.sars$`Regulation.SARS vs Mock.2.s.log2FC`
delta.x <- anal.table.sars$`Regulation.SARS vs Mock.2.HL.log2FC`

# If I'm using the 2h simulation
sigma.y <- anal.table$`Regulation.s vs control.s.log2FC`
delta.y <- anal.table$`Regulation.HL vs control.HL.log2FC`

# If I'm using the 2h subset of the real data
# Something similarly named
```

Fig5:

Plot for synthesis rates:

```{r}
df.sigma <- cbind(sigma.x, sigma.y)
PlotScatter(df.sigma)
```

Plot for degradation rates:

```{r}
df.delta <- cbind(delta.x, delta.y)
PlotScatter(df.delta)
```
