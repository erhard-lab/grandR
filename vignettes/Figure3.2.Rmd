...
plot_dist <- function(RDS,state = 'ss',cols = 'Half-life'){
...
ggplot(tabm, aes(x=diff_relative, color=method, linetype = method)) + 
   stat_ecdf(geom = "line")+ coord_cartesian(xlim=c(-0.5,0.8)) + xlab('log2 fold change') + ylab('cumulative frequency') + geom_vline(xintercept = 0)+
...
plot_dist(data_default, state = 'ss', cols = 'Half-life')
plot_dist(data_default,state = 'noss', cols = 'Half-life')
```
...
# Plot_Scatter_CI_size <- function(RDS,state,cols) {
# size = data.frame('ntr'= confidence_size(method='ntr',RDS=RDS,state = state,cols=cols),
...
# p1 = PlotScatter(size,1,2) + geom_abline()
# p2 = PlotScatter(size,1,3) + geom_abline()
# p3 = PlotScatter(size,1,4) + geom_abline()
# p4 = PlotScatter(size,2,3) + geom_abline()
# p5 = PlotScatter(size,2,4) + geom_abline()
# p6 = PlotScatter(size,3,4) + geom_abline()
...
ggplot()+geom_boxplot(data=tabm, aes(x=variable,y=value))+xlab('method')+ylab('size of confidence intervals')+coord_cartesian(ylim=c(-100,100))
}

# Plot_Heat_CI_size <- function(RDS,state,cols) {
#   tab = GetAnalysisTable(RDS,columns=cols)
#   size = data.frame('gene' = rownames(tab),
#                     'ntr'= confidence_size(method='ntr',RDS=RDS,state = state,cols=cols),
#            'entr'=confidence_size(method='entr',RDS=RDS,state = state,cols=cols),
...
#            'lm'= confidence_size(method='lm',RDS=RDS,state = state,cols=cols))
# tabm = melt(size, id.vars = 'gene')
# ggplot(tabm, aes(gene, variable, fill= log(value)))+ geom_tile() + scale_fill_gradient(low="white", high="red")
# ggplot(tabm, aes(gene, variable, fill= log(value)))+ geom_tile() + scale_fill_gradient(low="white", high="red") 
# }
```

```{r}
#Plot_Scatter_CI_size(data_default,'ss','Half-life')
#Plot_Scatter_CI_size(data_default,'noss','Half-life')
Plot_Box_CI_size(data_default,'ss','Half-life')
Plot_Box_CI_size(data_default,'noss','Half-life')
#Plot_Heat_CI_size(data_default,'ss','Half-life')
```
```{r compare tps and replicates, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8,fig.height=4}
...
#legend_order = levels(with(tabm,reorder(tps,ltp)))
...
#stat_ecdf(geom = "line")+ xlim(-0.5,0.8) + xlab('log2 fold change') + ylab('cumulative frequency')+geom_vline(xintercept = 0) 
# + facet_wrap(ltp~reps, labeller = labeller(ltp = label_facet(tabm$ltp, 'timepoints'), reps = label_facet(tabm$reps, 'replicates'))) + scale_color_discrete(name = 'timepoints', breaks = legend_order)

#ggplot(tabm, aes(x=diff_relative, color=tps)) +
#stat_ecdf(geom = "line")+ xlim(-0.5,0.8) + xlab('log2 fold change') + ylab('cumulative frequency')+geom_vline(xintercept = 0)  + facet_wrap(ltp~reps, labeller = labeller(ltp = label_facet(tabm$ltp, 'timepoints'), reps = label_facet(tabm$reps, 'replicates'))) + scale_color_discrete(name = 'timepoints', breaks = legend_order)
...
#,limits=c(-0.5,0.5))+
 coord_cartesian(ylim = c(-0.5,2.15))+scale_y_continuous(breaks = seq(from = -0.5, to =2.15, by = 0.1))+
  theme(axis.text.x = element_text(size=12, angle=90, hjust = 1, vjust = 0.4), axis.text.y = element_text(size=10)) + ylab('log FC true HL/inferred HL') + xlab('')
```
...
tabm$reads = sapply(c(1:length(tabm$method)), function(i) reads[as.numeric(strsplit(as.character(tabm$method[i]),'[.]')[[1]][2])])
tabm$reads = factor(as.character(tabm$reads), levels = c('5E5','1E6','1E7','2E7','4E7','5E7'))
ggplot(tabm, aes(x=diff_relative, color=reads, linetype=reads)) + stat_ecdf(geom = "line")+ coord_cartesian(xlim=c(-0.5,0.8)) + xlab('log2 fold change') + ylab('cumulative frequency') +geom_vline(xintercept = 0)
```
...
tab = data.frame('original'=Coldata(df,'duration.4sU.original'),Replicate=Coldata(df,'Replicate'),Condition=Coldata(df,'Condition'),'calibrated_30'=Coldata(df,'calibrated_time'),'calibrated_35'=Coldata(df2,'calibrated_time'),'calibrated_25'=Coldata(df3,'calibrated_time'))
tabm = melt(tab, id.vars = c('original','Replicate','Condition'))
#names(tabm) = c('x','c','y')
ggplot(tabm,aes(original,value,color= variable, group = Replicate))+geom_point(position = position_dodge(width=0.7)) + facet_wrap(~Condition) + theme_minimal_hgrid()
```
