...
`PlotScatter` can also display results from analyses (see the [differential expression](differential-expression.html) and [kinetic modeling](kinetic-modeling.html) vignettes for more information on analyses). To demostrate, we first create some analyses results:
...
In addition to this first page, the web interface now has a second tab pabel to show the scatter plot. This lets you select genes by drawing rectangles with the mouse:
