...
At 3h post infection, 4sU was added to the cells for 1,2,3 or 4h. The two no4sU samples are also 3h post infection. The infection time point can be added to this table via:
...
For the NNLS approach it is important to normalize the data:
```{r}
sars<-Normalize(sars)
sars
```
...
```{r,fig.height=3.5,fig.width=5}
PlotGeneKinetics(sars,"SRSF6",steady.state=list(Mock=TRUE,SARS=FALSE))
```
Note that the fit actually changed. Now we are ready to fit the model for each gene. For that, we set `SetParallel()` which will set the number of worker threads to "number of cores"-2. We also specify the same `steady.state` parameter to `FitKinetics`:
...
So far we do not see any results. The reason is that the result table has only been added to the grandR object:
```{r}
sars
```
...
```
This tells us that there are indeed two analysis tables, each having a Synthesis and a Half-life column. We can also retrieve the analysis results using the `GetAnalysisTable` function:
```{r}
...
```{r}
small <- FilterGenes(sars,use=1:100)             # let's perform analyses on fewer genes, it's faster
small<-FitKinetics(small,name = "with.loglik",steady.state=list(Mock=TRUE,SARS=FALSE),return.fields=c("Synthesis","Half-life","logLik"))
...
```{r}
small<-FitKinetics(small,name = "with.loglik",steady.state=list(Mock=TRUE,SARS=FALSE),return.extra=function(d) c(lower=d$conf.lower["Half-life"],upper=d$conf.upper["Half-life"]))
Analyses(small,description = TRUE)
```
...
df <- melt(df,variable.name = "Sample",value.name = "Relative residual",id.vars = c())
df$Sample=gsub("Residuals.","",df$Sample)
ggplot(df,aes(Sample,`Relative residual`))+geom_boxplot()+RotatateAxisLabels()+geom_hline(yintercept = 0)+coord_cartesian(ylim=c(-1,1))
```
We see that the residuals from the 1h timepoints are systematically below 0. A canonical example of this is SMAD3:
```{r}
PlotGeneKinetics(sars,"SMAD3",steady.state=list(Mock=TRUE,SARS=FALSE))
```
For both Mock and SARS, we see that new RNA from 1h is well below the model fit, i.e. has negative residual. This indicates that the effective labeling time was much lower than the nominal labeling time of 1h for these samples, i.e. that actualy, these samples should move to the left. This can be rectified by temporal recalibration:
...
```{r}
PlotGeneKinetics(sars,"SMAD3",steady.state=list(Mock=TRUE,SARS=FALSE),time = "calibrated_time",exact.tics = FALSE)
```
