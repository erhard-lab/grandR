...
df <- cbind(GetTable(sars,c("kinetics.Mock"),columns="Half-life"),GetTable(sars,c("kinetics.SARS"),columns="Half-life"))
# REVIEWED BY Lygeri: added distinct colnames for Mock and SARS
colnames(df) <- c("Mock.Half-life", "SARS.Half-life")
...
df<-cbind(GetTable(sars,c("kinetics.Mock"),columns="rmse"),GetTable(sars,c("norm_kinetics.Mock"),columns="rmse"))
PlotScatter(df,xcol=1,ycol=2,log=T)+geom_abline()
```

```{r}
df<-cbind(
  GetTable(sars,c("kinetics.Mock"),columns="Half-life"),
  GetTable(sars,c("norm_kinetics.Mock"),columns="Half-life"),
  GetTable(sars,c("ntr_kinetics.Mock"),columns="Half-life"),
  GetTable(sars,c("normntr_kinetics.Mock"),columns="Half-life"),
  GetTable(sars,c("lm_kinetics.Mock"),columns="Half-life")
  )
