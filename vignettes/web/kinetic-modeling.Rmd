...
```{r,fig.height=4, fig.alt = "PCA plot Mock vs SARS"}
PlotPCA(sars)
```
...
```{r,fig.height=4, fig.alt = "time points"}
...
```{r,fig.height=3.5,fig.width=5, fig.alt = "Progressive labeling plot for SRSF6"}
...
```{r,fig.height=3.5,fig.width=5, fig.alt = "Progressive labeling time plot for SRSF6 with correct steady state configuration"}
...
```{r warning=FALSE, fig.alt = "Scatter plot comparing RNA half-lives in Mock and SARS"}
...
```{r fig.alt = "Residual distribution of fits across all genes"}
...
```{r, fig.alt = "Progressive labeling plot for SMAD3"}
...
```{r, fig.alt = "Progressive labeling time plot for SMAD3 after recalibration of effective labeling times"}
PlotGeneProgressiveTimecourse(sars,"SMAD3",steady.state=list(Mock=TRUE,SARS=FALSE),
...
```{r fig.alt = "Residual distributions after recalibration"}
