...
```{r,fig.height=4, fig.alt = "Uncolored PCA plot"}
PlotPCA(banp)
```
...
banp <- LFC(banp,name.prefix = "QC",contrasts=contrasts, compute.M = FALSE)
banp
```

As you see, now we have two analyses added to our grandR object. We can use `PlotScatter` to directly plot them:

```{r fig.alt = "QC scatter plot of 4sU effect vs dTAG effect"}
...
```{r fig.height=4, warning=FALSE, fig.alt = "4sU dropout plots"}
...
```{r fig.alt = "Clusterprofiler ridge plot"}
...
```{r fig.height=3, fig.width=3,  fig.alt = "Vulcano of dTAG effect"}
...
```{r fig.height=3, fig.width=6, warning=FALSE, fig.alt = "MA plots of 1h and 2h vs 0h"}
...
```{r fig.height=3, fig.width=6, warning=FALSE, fig.alt = "MA plot of 4h vs 0h (total and new RNA)"}
...
```{r fig.height=3, fig.width=5, fig.alt = "Boxplots of log FC distributions for targets and non-targets"}
...
```{r, fig.alt = "Boxplots of log FC distributions for targets and non-targets in different HL categories"}
...
```{r fig.alt = "Boxplots of half-lives per time point"}
...
```{r fig.alt = "Boxplots of half-lives per time point after calibrating effective labeling times"}
...
```{r fig.height=7, fig.width=7, fig.alt = "Scatter plots log FC synthesis vs HL at different time points"}
