...
Here we see how to work with the metadata for genes. The gene metadata essentially is a table that can be retrieved using the `GeneInfo` function:
...
For inferring the `Type` column, the `GeneType()` function is used. By default, this will recognize mitochondrial genes (MT prefix of the gene symbol), ERCC spike-ins, and Ensembl gene identifiers (which it will call "cellular"). Here, we also have the viral genes, that are not recognized:
...
Note that each parameter to `GeneType` must be named (`viral`) and must be a function that takes the gene metadata table and returns a logical vector. 

The `GeneType` function has one additional important parameter, which defines how "Unknown" types are supposed to be called. For this data set, a similar behavior as above can be accomplished by:
