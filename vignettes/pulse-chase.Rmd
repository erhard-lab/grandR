...
In this step, we need to modify the sample names in the data, as the names were not processed using systematic naming conventions. If systematic sample names were used, this step is not necessary.
To do this, we use the rename.sample parameter in the `ReadGRAND` function, which allows us to specify a function for renaming the samples.
The specified function removes all instances of "chase" and replaces 0.5 with 0_5. For example, "mESC.0.5h.chase.B" becomes "mESC.0_5h.B" and "mESC.0h.chase.C" becomes "mESC.0h.C". The output of the function is the modified vector after these replacements. This enables us to load the data using the design vector.

```{r}
d=ReadGRAND("/mnt/hilbert/projects/erhard/grandslam/ameres/chase_notrescued/grandslam_t15_fixedparam/chase_notrescued",
            design=c(NA,"Time",Design$Replicate),rename.sample=function(v) gsub(".chase","",gsub("0.5h","0_5h",v)),read.percent.conv = TRUE)
d=FilterGenes(d)
d=Normalize(d)
```
...
```
We can also inspect parameters for single genes for example:

```{r}
PlotGeneProgressiveTimecourse(d,gene=grep("Nanog",d$gene.info$Gene),time="Time",type="chase")
```

The curves represent the fitted model for this gene. 
...
```{r,warning=FALSE}
t=read.delim("/mnt/hilbert/projects/erhard/grandslam/ameres/chase/from_paper/halflives.tsv")
t$Gene=paste0(t$Name,".",t$End)
d = AddAnalysis(data = d,"kinetics",table = t,by="Gene")
PlotScatter(d,`kinetics.Half.life..h.`,`kinetics.Half-life`)+geom_abline()
```
